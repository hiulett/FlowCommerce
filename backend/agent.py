...
async def run_conversational_agent(
    ...
    system_prompt = (
        ...
        f"- Si la herramienta 'confirm_and_checkout_order' te devuelve un mensaje indicando que la compra falló por falta de stock de algún producto u otro error, DEBES informarle al cliente EXACTAMENTE qué producto falló (usando el mensaje que te dio la herramienta). Luego, pregúntale si desea eliminar o reemplazar dicho producto. NO respondas solo con 'hubo un inconveniente', sé específico y claro sobre la causa del error.\n"
