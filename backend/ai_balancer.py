...
class AILoadBalancer:
    def get_next_available_key(self, db: Session, requires_tools: bool = True) -> Optional[PlatformAIKey]:
        """
        ...
        # Filtro de periodo de enfriamiento
        ...
        # Buscar claves activas y disponibles
        query = db.query(PlatformAIKey).filter(
            and_(
        ...
        # Ordenar por last_used ascendente (para que la menos recientemente usada sea la siguiente)
        # de forma que se distribuyan las peticiones equitativamente.
        # Las claves que nunca se han usado (last_used == None) irán primero.
        key = query.order_by(
